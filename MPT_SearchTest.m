...
th_ = unifrnd(-err(1),err(1),8,1e4);

setA = cell(Num_regions,1);
for w = 1:Num_regions
    setA{w,1} = R.xopt.Set(w,1).A;
end

setb = cell(Num_regions,1);
for w = 1:Num_regions
    setb{w,1} = R.xopt.Set(w,1).b;
end

Fi=zeros(3,8,Num_regions);
for w = 1:Num_regions
    Fi(:,:,w) = R.mpqpsol.Fi{1,w};
end
Gi=zeros(3,1,Num_regions);
for w = 1:Num_regions
    Gi(:,:,w) = R.mpqpsol.Gi{1,w};
end

save('TestTime.mat','Num_regions','MaxTable','MinTable','setA','setb','Fi','Gi')
	tic  
for k = 1:1e4
    Realth = ones(Num_regions,1) * th_(:,k)';
    ...
    if size(Credit,1)==1
        ...
        for p = Credit'
            if sum(setA{p}*th_(:,k) <= setb{p}) == size(setA{p},1)
    			% j
                break
            end
        end
